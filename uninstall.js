...
const binp = path.join(cargoDir, "bin", "fauxd");

if (fs.existsSync(binp)) {
  console.log("Uninstalling fauxd...");
  exec(`cargo uninstall fauxd`, (error, stdout, stderr) => {
    console.log(stdout);
    if (error || stderr) {
  ...
  console.log("fauxd not found skipping!");
}
